...
/**
 * 数组 + 单向链表（链表上的元素多的时候可转换为红黑树，当红黑树元素数量多的时候，又可以退化为链表）
 * 同一个 key 的 hashcode 和 equals 必须重写
 * hash =  return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);
 * 通过数组长度和hashcode 算出 key 对应在数组的索引位置((length - 1) & hash)
 */
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
     ...
     * map 的负载因子
     * <p>
     * 当 Map 中的元素达到总容量的 0.75 时候触发扩容
     */
    static final float DEFAULT_LOAD_FACTOR = 0.75f;

    /**
     * 当数组上的链表长度达到 8 的时候,链表转换为红黑树
     */
    static final int TREEIFY_THRESHOLD = 8;

    /**
     * 红黑树上节点小于 6 时，红黑树退化为链表
     */
    ...
    /**
     * 当 Map 的下一个 size 达到 (capacity * load factor),触发扩容及 rehash
     * 元素数量达到这个值，触发库容
     */
    int threshold;
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        if ((p = tab[i = (n - 1) & hash]) == null) {
            ...
            if (e != null) {
                V oldValue = e.value;
                // 当 key 对应的值不存在,或者对应值为 null 时才替换值
    ...
    private void putMapEntries(Map<? extends K, ? extends V> m, boolean b) {
