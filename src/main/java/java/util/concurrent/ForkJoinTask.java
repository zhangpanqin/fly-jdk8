...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
            ...
            if (t instanceof ForkJoinWorkerThread) {
                ...
                while ((s = status) >= 0 && (ns = deadline - System.nanoTime()) > 0L) {
                    if ((ms = TimeUnit.NANOSECONDS.toMillis(ns)) > 0L && U.compareAndSwapInt(this, STATUS, s, s | SIGNAL)) {
                        synchronized (this) {
                            if (status >= 0) wait(ms); // OK to throw InterruptedException
                            else notifyAll();
                        }
                    }
                }
    ...
    public static int getQueuedTaskCount() {
        ...
        if ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) q = ((ForkJoinWorkerThread) t).workQueue;
